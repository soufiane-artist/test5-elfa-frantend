...
function EditPub({user}) {
    ...
    useEffect(()=>{
        const getUserById =async()=>{
            await axios.get(`${process.env.REACT_APP_API_URL}/api/auth/publicier/`+postId)
            .then((res)=>{
                setPost(res.data)
            }).catch((err)=>{
                console.log(err);
    ...
    const Post =async()=>{
        ...
        await axios.put(`${process.env.REACT_APP_API_URL}/api/auth/publicier/`+postId,{
            title :title,
